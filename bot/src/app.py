...
async def start_flat_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rooms = f"Комнат: {data['params']['Количество комнат']}"
    price = f"Цена: {data['price']}"
    answer = f"*{data['title']}*\n\n🌇{data['adres']}\n\n{rooms}\n\n{price}\n\n{link}"
    context.user_data['index_falt'] = data['loc']
    print('--------start_flat_selection--------', context.user_data['index_falt'])
...
async def like(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print('---like----', context.user_data['index_falt'], '----old----')
    predict_like_flat(context.user_data['index_falt'])
